...
export default function EnterCitySubmit() {
  const theme = ClientCookieExtractor.extractTheme()
  const inputId = useId()
  ...
  return (
    <div className='relative'>
      <input
        ...
        value=''
        type='submit'
        className='ml-auto h-full w-[45px] cursor-pointer bg-gray-600 hover:bg-gray-500 active:bg-gray-700'
      />
      <label htmlFor={inputId}>
        <svg
          ...
          className='absolute left-[50%] top-[50%] z-0 translate-x-[-50%] translate-y-[-50%] transform cursor-pointer'
        >
          <path
            ...
            fill={
              theme ? (theme === Theme.DARK ? '#37352F' : 'white') : '#37352F'
            }
          />
        </svg>
      </label>
